...
merged_data<-final_table[is.na(final_table$CPIAUCNS),]
```
...
The "best" directors actually have a below average revenue, but the "mid-tier" directors do seem to have above average revenue for their movies.

```{r echo=FALSE}

p1 <- ggplot(final_table, aes(x=factor(director_score))) + geom_bar() + ggtitle("Distribution of Director Score") + xlab("Month") + ylab("Count")
...
The global box office is not evenly distributed. While this has changed over the years, the american audience still makes up the biggest chunk of the global box office.
...
p1 <- ggplot(final_table, aes(x=log(profitability_ratio))) + geom_histogram(bins = 100) + ggtitle("Distribution of Log of Profitability Ratios") + xlab("Log of Ratio") + ylab("Count")
...
p2 <- ggplot(final_table, aes(x=runtime)) + geom_histogram(bins = 100) + ggtitle("Distribution of Film Runtime") + xlab("Runtime (min)") + ylab("Count")
...
plot(log(final_table$budget), log(final_table$revenue), main = 'Log Revenue over Budget', xlab = 'Log of Budget', ylab = 'Log of Revenue')
...
plot(final_table$runtime, log(final_table$revenue), main = 'Log Revenue over Runtime', xlab = 'Runtime', ylab = 'Log of Revenue')
