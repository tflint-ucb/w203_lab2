...
d <- read.csv('./src/data/TMDB Data Set.csv')
...
<<<<<<< HEAD
```{r exclude variables, include=FALSE}
summary(d)
...
excluded_variables <- names(d) %in% c("homepage", "overview", "imdb_id", "popularity", "poster_path", "status", "tagline", "title", "Keywords")
...
# Convert belongs_to_collection to a True or False variable
by(clean_data, seq_len(nrow(clean_data)), function(row) {
  genres <- fromJSON(row$genres)
  sink(genres)
})
```
