...
clean_data<-clean_data[(clean_data$genres != ""),]
clean_data<-clean_data[(clean_data$cast != "[]"),]
...
clean_data <- clean_data %>%
 mutate(
   genres = map(genres, function(genre) {
      genre <- gsub("'(?![a-z])|(?<=\\{|\\s)'", '"', genre, perl = TRUE)
      cat(genre, '\n')
      parsed <- fromJSON(genre)
      return(parsed$name)
    }),
    cast = map(cast, function(full_cast) {
      coded <- gsub('None', 0, full_cast)
      parsed <- gsub("'(?![a-z])|(?<=\\{|\\s)'", '"', coded, perl = TRUE)
      cat(genre, '\n')
